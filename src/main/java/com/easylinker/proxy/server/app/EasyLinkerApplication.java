...
public class EasyLinkerApplication implements CommandLineRunner {
            ...
            if (clientsArray.size() > 4) {
                ...
                for (Object o : clientsArray) {
                    ...
                    if (isNumeric(clientJson.getString("client_id"))) {
                        ...
                        Long deviceId = clientJson.getLongValue("client_id");
    ...
    public void checkOfflineDevice() {
        try {
            ...
            List<Long> idList = deviceService.getAllId();
            for (Long deviceId : idList) {
                if (!emqDeviceIdList.contains(deviceId)) {//如果数据库里面的ID不在EMQ连接的上面,那说明就掉线跑飞了
                    Device device = deviceService.findADevice(deviceId);
                    if (device != null) {
                        if (device.isOnline()) {
