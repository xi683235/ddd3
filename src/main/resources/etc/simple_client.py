...
class SDK(threading.Thread):
    ...
    #消息回显
    def echo(self, data):
        self.client.publish("IN/ECHO/"+self.key.split("-")
        [0]+"/"+self.key.split("-")[1]+"/"+self.key.split("-")[2], str(data))
    #发布消息
    def publish(self, data):
        self.client.publish("IN/DEVICE/"+self.key.split("-")
        [0]+"/"+self.key.split("-")[1]+"/"+self.key.split("-")[2], str(data))
...
if __name__=="__main__":
    sdk= SDK("localhost",1883,"1523858186999-1528853267978-1528854199408",on_message,True)
    sdk.start()
    #数据上传
    while 1:
        time.sleep(10)
