# EasyLinker-部署和使用文档
## V1.0效果图:https://www.oschina.net/p/easylinker
...
>>为了方便大家学习，我们准备了一些学习资料:
 ...
 >>- 物联网系列课程:http://www.easylinker.xyz/
 >>- 学习文档:http://www.easylinker.xyz/
 >>- EasyLinkerAPP地址:http://www.easylinker.xyz/
 >>- 定制版EMQ下载地址:http://www.easylinker.xyz/
 >>- EasyLinker架构图和应用场景讲解:http://www.easylinker.xyz/
 >>- 特别录制了一个部署安装小教程视频:http://www.easylinker.xyz/
...
# 六·快速写一个客户端  
>###1.python-SDK demo
>>安装SDK:```pip install easylinker-sdk```,注意，项目依赖库是:```paho-mqtt```,请先安装.
```python
#这里是你的设备的信息
CLIENT_OPENID="1522646205561"
CLIENT_GROUP="1522648770819"
CLIENT_USER_ID="1522567164523"    
#消息响应函数
def on_message(client, userdata, msg):
    print("Received Data:",msg.payload)

if __name__=="__main__":
    #构造SDK,服务器和端口取决于你的实际情况
    sdk= SDK("localhost",1883,CLIENT_OPENID,CLIENT_GROUP,CLIENT_USER_ID,on_message)
    sdk.start()
    #需要上传数据的时候，只需要吧JSON传进 publish(JSON 格式的数据) 这个函数就可以了
    #如下Demo
    time.sleep(5)
    sdk.publish(data={"temp":"1"})   
    time.sleep(5)
    sdk.publish(data={"hum":"122"})  
    time.sleep(5)
    sdk.publish(data={"w":"34"})   
```
>### 2.测试通信
>>用HTTP接口==>```POST:localhost:2500/api/v1/sendCmdToDevice```
```json
{
	"deviceId":1522646205561,
	"payload":{
		"title":"helloworld"
	}
}
...
>###API文档请看这里:https://documenter.getpostman.com/view/1700622/proxyserver_01/RVtvqt6z
# 六·二次开发  
>###1.技术架构
>>1.Springboot2.0:EasyLinker的核心技术框架
>>2.SpringDataJPA:持久层框架
>>3.SpringSecurity作为逻辑层安全组件
>>4.EMQ：百万级物联网消息服务器
>###2.项目结构  
```
├─.idea
...
```
> ## 3.模块介绍  
>>####EasyLinker:核心业务逻辑层，整个用户管理，设备管理逻辑，全部通过它完成。
>>####EMQ：消息中转服务器。

> ## 4.功能扩展  
>>如果需要扩展业务逻辑层，只需要增加Controller就可以
>>如果增加模型，则需要在Model层中增加SpringDadaJPA的Entity.
# 七·一个小案例:智能温度控制  
>>http://www.easylinker.xyz/?t/20.html